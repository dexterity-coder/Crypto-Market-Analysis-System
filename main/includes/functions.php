...
function upload_web_image($path, $ext) {
    $img_url = 'img_' . date('mdYHis.') . $ext;
    move_uploaded_file($path, "temp_img/" . $img_url);

    $resizeObj = new resize("temp_img/" . $img_url);
    $resizeObj->resizeImage(1319, 640, 'exact');
...
//Upload and Crop passport images 
function upload_passport_image($path, $ext, $sn) {
    $img_url = 'passport' . $sn . '-' . date('mdYHis.') . $ext;
    move_uploaded_file($path, "temp_img/" . $img_url);

    $resizeObj = new resize("temp_img/" . $img_url);
    $resizeObj->resizeImage(413, 531, 'exact');
    $resizeObj->saveImage("images/" . $img_url, 100);
    unlink("temp_img/" . $img_url);
    return $img_url;
}
